import WeakMap from "https://unpkg.com/@ungap/weakmap@^0.1.4?module";
import tta from "https://unpkg.com/@ungap/template-tag-arguments@^0.3.1?module";
import { augmentor } from "https://unpkg.com/dom-augmentor@^2.0.7?module";
import { isArray } from "https://unpkg.com/uarray@^1.0.0?module";
import umap from "https://unpkg.com/umap@^1.0.2?module";
...
"https://unpkg.com/lighterhtml@^3.0.0?module";
...
"https://unpkg.com/dom-augmentor@^2.0.7?module";
...
const unrollValues = ({ stack }, values, length) => {
  for (let i = 0; i < length; i++) {
    ...
    values[i] = unroll(stack[i] || (stack[i] = createCache()), hook);else
    if (hook instanceof Hole)
    unrollHole(stack[i] || (stack[i] = createCache()), hook);else
    if (isArray(hook))
    unrollValues(stack[i] || (stack[i] = createCache()), hook, hook.length);else
